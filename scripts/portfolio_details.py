...
<div class="source-box"><strong>检查这个项目</strong>{ext(blog,'完整开源仓库')}{ext(blog+'/blob/master/api/src/services/content.ts','发布状态与内容读取源码')}{ext(blog+'/blob/master/docs/DEPLOYMENT.md','部署与验收说明')}</div>''', 'projects', '<p class="eyebrow">我的角色</p><p>项目发起者 / 产品定义<br>AI 协作开发与验收</p><p class="eyebrow">技术</p><p>Next.js · TypeScript<br>Hono · Drizzle · PostgreSQL</p><p class="eyebrow">阶段</p><p>已部署、自用<br>源码公开 · MIT</p>')
    ...
    article('projects/shell.html', 'PROJECT 03 · AGENT 交互', 'Agent 手机接入原型：登录与连接已验证，完整体验待验收', '基于 DeepSeek Harness 开发交互外壳与局域网登录网关。已有连接层实验；手机端从登录、对话到工具执行的完整用户流程尚不能据此认定完成。', f'''
<section class="editor-note"><strong>验收范围：已完成部分能力验证，尚未证明完整手机 Agent 闭环。</strong><p>401 / 403 / 101 说明网关鉴权、Host 校验和 WebSocket 握手结果，不能替代一次真实用户任务。</p></section><h2>哪些链路跑过，哪些还没有充分证据</h2><div class="table-scroll"><table><thead><tr><th>链路</th><th>已有证据</th><th>仍需区分的边界</th></tr></thead><tbody><tr><td>LAN 登录与连接</td><td>公开 README 有拒绝未登录、Host 校验及连接成功实验</td><td>只证明连接层，不证明手机对话和工具结果</td></tr><tr><td>桌面文字回复与语音输出</td><td>2026-08-25 的本地项目记录记载过回复到语音、口型的验证</td><td>历史私有实例记录，不是公开仓库的手机验收报告</td></tr><tr><td>独立实时语音通话</td><td>2026-09-03 的本地记录记载自动化链路测试与用户试用通过</td><td>通话与工作台文字链路分开；语音通话没有接入工作台工具</td></tr><tr><td>手机完整 Agent 任务</td><td>本页未提供覆盖完整流程的验收材料</td><td>需在真实手机验证登录、输入、回复、工具执行与结果，另核对语音及恢复行为</td></tr></tbody></table></div><p>以上区分历史记录与公开证据，不代表本次重新运行了桌面、手机或语音服务。</p><h2>我想解决的使用问题</h2><p>Agent 在电脑上运行，但我希望能从手机进入同一个交互环境。增加一个页面并不足够：登录、可信连接、会话消息以及上游的能力入口都需要一起工作。</p>
...
<h2>不能从中推出什么</h2><p>这不是完整 Agent 评测平台，没有证明长期自主运行、多租户隔离或全部工具链可靠。持续存储、广泛 Provider 集成和学习机制仍是公开路线图中的待办。我的下一步需要围绕真实任务继续验证，而不是把最小实验包装为成熟系统。</p><div class="source-box"><strong>检查实验</strong>{ext(runtime+'/blob/main/tests/test_runtime.py','测试与断言')}{ext(runtime+'/blob/main/src/organic_agent_os/runtime.py','事件执行与回执源码')}{ext(runtime+'/blob/main/ROADMAP.md','尚未完成的工作')}</div>''')
    ...
    page('notes/index.html','实践笔记','每篇笔记对应我自己的项目、使用场景或开源实验。', f'''<section class="wrap listing"><p class="eyebrow">FIELD NOTES / 09</p><h1>我读过什么，<br>也写清楚<span class="accent">我怎么用。</span></h1><p class="listing-lead">从原博客的学习积累中，选出能接到个人实践的主题。<br>每篇开头都给出对应项目、当前阶段与公开证据。</p><div class="editor-note">这里有项目复盘、开源提交，以及从原博客重新整理的应用笔记。研究过一个框架不等于已经部署它；每篇按实际完成的范围说明。</div><div class="notes-section">{note_list()}</div></section>''','notes')

    page('about.html','关于与联系','黄智军 的求职方向、个人贡献、项目入口与直接联系方式。',f'''<section class="wrap about-page"><p class="eyebrow">ABOUT 黄智军</p><h1>既关心产品怎么用，<br>也愿意把它<span class="accent">做出来。</span></h1><p class="about-intro">我关注 AI 产品、Agent 应用和知识系统。以自己的真实需求作为起点，借助 AI 做产品定义、实现、部署和持续改进。公开账号是 xmhuangzhijun-hue。</p><div class="about-columns"><div><h2>主方向：FDE</h2><p><strong>FDE（前线部署工程师）：</strong>理解客户或业务现场的工作流程与限制，集成已有系统，推进数据、应用与 AI 能力落地，并完成可维护的交付。</p><h3>延伸方向</h3><p><strong>AI 产品定义：</strong>拆解用户任务、设计功能流程、定义状态和验收标准，推动从原型到可用产品。</p><p><strong>AI 应用开发：</strong>集成开源 Agent、构建 Web 与 API、处理数据持久化与部署问题，并把可复现的修复提交上游。</p><h2>一项外部可验证的贡献</h2><p>Hermes 的原始修复 #97572 已由维护者保留署名，经 #101570 整合合入。<a href="contributions.html">查看原始 PR 与合入记录 →</a>。自有参考运行时的 5 个测试单独呈现，不计作上游贡献。</p><h2>我的协作方式</h2><p>需求、优先级和关键取舍由我负责；调研、编码与调试使用 AI 协作。底层框架、我发起的项目、个人提交和维护者后续改动分别注明。</p></div><div><h2>可以先看这些工作</h2><ol><li><a href="experience.html">工作经历：投放中台与数据看板</a></li><li><a href="projects/blog.html">个人博客：从需求到内容发布系统</a></li><li><a href="notes/vision-timeout.html">Hermes 修复：从使用问题到上游采纳</a></li><li><a href="projects/shell.html">Agent 外壳：从电脑界面到手机接入</a></li></ol><p><a href="skills.html">按项目查看技能分类 →</a></p><h2>其他探索</h2><p><a href="projects/memory.html">Agent 记忆与知识管理</a>展示我的长期实践和参考实现；<a href="projects/builder.html">Agent 创建平台</a>目前是产品方案与本地底座，作为补充材料保留。</p><p>{ext(gh,'查看对外 GitHub 账号')}</p></div></div><div class="about-end"><a class="button primary" href="#contact">直接联系我 ↓</a><a class="button" href="notes/index.html">阅读实践笔记</a></div></section>''','about')
