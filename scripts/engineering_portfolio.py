...
def project_evidence(slug):
    facts = {
      ...
      'assistant': ('Hermes 个人助手实例 · 场景集成与上游贡献', [
        ('产品问题','个人信息散落在聊天、日记和知识库中；重复解释背景，且自然对话与实际记录之间缺少可靠连接。'),
        ('我的职责','定义使用场景与产品原则，整理 Obsidian 资料、上下文和记忆边界，借助 AI 接入运行时与工具，并按实际使用结果验收。'),
        ('技术实现','Hermes 实例与原生 MemoryProvider；微信入口；Obsidian/Markdown；Python、MCP 与 SQLite。OpenClaw 适配单独核对，不混称所有运行时功能等价。'),
        ('核心取舍','模型负责语义与工具选择，代码负责权限、状态、幂等和结果核验；资料、事实与运行时缓存分层，支持后续迁移。'),
        ('公开证据','个人助手使用中发现的视觉超时修复经 #101570 保留署名合入；下文公开搭建流程与设计依据，完整个人实例不等同公开 Fork。')]),
      'shell': ('自有扩展项目 · 复用 DeepSeek Harness', [
        ...
        ('可验证结果','公开实验区分未登录 401、Host 未获信任 403、连接成功 101；仅证明连接层；手机对话、工具执行和恢复流程尚无完整验收证据。')]),
